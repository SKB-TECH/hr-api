...
export class CandidateEducationController {
  ...
  async create(@Req() req, @Body() dto: CreateEducationDto) {
    ...
    return sendResult(HttpStatus.CREATED, 'Education added', data);
  }
  @Get(':candidateId')
  @ApiOperation({
  ...
  @ApiParam({
    ...
    format: 'uuid',
    description:
      'The unique identifier of the candidate whose education records are being retrieved',
  ...
  async findCandidateEducation(
    ...
    return sendResult(HttpStatus.OK, 'Education fetched', data);
  }
  ...
  async update(
    ...
    return sendResult(HttpStatus.OK, 'Education updated', data);
  }
  ...
  async remove(@Req() req, @Param('id', ParseUUIDPipe) id: string) {
    ...
    return sendResult(HttpStatus.OK, 'Education deleted', data);
  }
}
