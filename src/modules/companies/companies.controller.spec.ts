...
describe('CompaniesController', () => {
  ...
  describe('findAll', () => {
    ...
    it('should forward all query filters to the service', async () => {
      ...
      const query = { search: 'Nomad', location: 'Paris', industry: 'Design', page: 1, limit: 12 };
    ...
    it('should return empty data when no companies match', async () => {
      ...
      const result = await controller.findAll({ search: 'nonexistent' });
  ...
  describe('findOne', () => {
    it('should return a single company', async () => {
      mockCompaniesService.findOne.mockResolvedValue(mockPaginatedResult.data[0]);

      const result = await controller.findOne('uuid-company-1');

      expect(result.id).toBe('uuid-company-1');
      expect(mockCompaniesService.findOne).toHaveBeenCalledWith('uuid-company-1');
    });
  });

  describe('update', () => {
    it('should call service update and return updated company', async () => {
      mockCompaniesService.update.mockResolvedValue({ ...mockPaginatedResult.data[0], name: 'Nomad Updated' });

      const result = await controller.update('uuid-company-1', { name: 'Nomad Updated' });

      expect(result.name).toBe('Nomad Updated');
      expect(mockCompaniesService.update).toHaveBeenCalledWith('uuid-company-1', { name: 'Nomad Updated' });
    });
  });
});
